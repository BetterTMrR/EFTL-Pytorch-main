...
def data_load(args): 
        ...
        for i in range(len(txt_test)):
            ...
            if int(reci[1]) in args.tar_classes:
                if int(reci[1]) in args.src_classes:
                    ...
                    line = prefix + reci[0] + ' ' + str(len(label_map_s)) + '\n'
                    new_tar.append(line)
    ...
    print('length of source tr: %s' % len(dsets['source_tr']))
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["source_te"] = ImageList(te_txt, transform=image_test())
    print('length of source te: %s' % len(dsets['source_te']))
    ...
    print('length of test dataset: %s' % len(dsets['test']))
...
def train_source(args):
    ...
    netB = network.feat_bootleneck(feature_dim=netF.in_features, bottleneck_dim=args.bottleneck).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--classifier', type=str, default="bn", choices=["ori", "bn", 'NM'])
    ...
    print('output_dir_src: %s' % args.output_dir_src)
